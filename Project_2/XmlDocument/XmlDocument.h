...
namespace XmlProcessing
{
	...
	public:
		...
		XmlDocument(const std::string& src, std::string srcType);
		XmlDocument(){ pDocElement_ = makeDocElement(); }
		XmlDocument& operator = (XmlDocument&& document);
		...
		std::string findTextInXmlComment(const std::string& s);
		void print(const std::string& s, bool condition);

		std::shared_ptr < AbstractXmlElement > DFS_r_ltr(sPtr pNode, std::string tagname); //depth first search
		std::shared_ptr < AbstractXmlElement > getpdocElement(){ return pDocElement_; }
		std::vector<sPtr> XmlDocument::element_vector(sPtr pNode, std::string tag);
		sPtr find_attribute(sPtr pNode, std::string tag);
		std::vector<sPtr> childrenAttribute(sPtr pNode, std::string tag);
		std::vector<std::pair<std::string, std::string>> attributeChildren(sPtr pNode, std::string tag);
		void processingDeclaration(std::string& part, XmlParts& parts, sPtr& dec);
		void processingStartingTag(std::string& part, XmlParts& parts, std::stack<sPtr>& s1);
